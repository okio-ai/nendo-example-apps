...
PROMPT_TEMPLATE = """Please use the following key and values 
of a description of an audio file 
to generate a summary of its contents. 
...
QUERY_PROMPT_TEMPLATE = """Please use the following key 
and values of a description of 
an audio file to generate a query to search for similar audio files. 
...
def build_simple_query(track: NendoTrack) -> str:
    ...
    Will use the moods, key, scale and instruments plugin data to build a query.
    An example query would be: "a happy song in C major with a guitar in it".

    Args:
        track (NendoTrack): The track to build the query for.
...
def get_text_from_plugin_data(track: NendoTrack) -> str:
    """Get a string representation of the plugin data of a track."""
    text = ""
...
def is_relevant(
    ...
    Args:
        ...
        at (int): The cutoff. Defaults to 1.
        threshold (int): The number of matches needed to be considered relevant.
            Defaults to 2.
...
def main():
    ...
    parser.add_argument(
        "--mode",
        default="embed",
        help="Which mode to run the program in. Either embed or eval.",
        choices=["embed", "eval"],
    )
    parser.add_argument(
        "--reset", action="store_true", default=False, help="Reset the nendo library."
    )
    parser.add_argument(
        "--subset",
        type=int,
        default=100,
        help="Number of audio files to evaluate the search on.",
    )
    parser.add_argument(
        "--relevancy_threshold",
        type=int,
        default=4,
        help="Number of matches between the original and the query result to be considered relevant.",
    )
    ...
    if args.mode == "embed":
        ...
        for track in tqdm(fma_small):
            try:
                if len(nd.library.get_embeddings(track_id=track.id)) > 0:
                    print(
                        f"Skipping track {track.id} because it has an embedding."
                    )
                    continue
                ...
                summary = nd.plugins.textgen(
                    prompt=PROMPT_TEMPLATE + get_text_from_plugin_data(track)
                )
                ...
                errors.append(
                    {"track_id": track.id, "path": track.resource.src, "error": str(e)}
                )
        ...
        for track in tqdm(eval_set):
            ...
            scores = is_relevant(
                track, tracks, at=[1, 5, 20], threshold=args.relevancy_threshold
            )
